...
def callback1(msg):
    try:
	    ...
	    if(((msg.models[0].pose.position.y)>-0.05) and ((msg.models[0].pose.position.y)<0.05)):
                    #print("********************* Avinash is a play boy **********************************************")
                constop()
                rospy.sleep(3) 
                constart()
	            
        elif (msg.models[1].type=="packagen2"):
            if((msg.models[1].pose.position.y<-0.1) and (msg.models[1].pose.position.y>-0.15)):
                constop()
                rospy.sleep(1) 
                constart()  
        elif (msg.models[1].type=="packagen3"):
            if((msg.models[1].pose.position.y<-0.08) and (msg.models[1].pose.position.y>-0.09)):
                constop()
                rospy.sleep(1) 
                constart() 
    	#print(msg.models[2].pose.position.y)
    except:
    	pass
def callback(msg):
    try:
        for i in range(3):
            print("msg.models{}".format(i)) 
	    print(msg.models[i])
            print('*******************************')
            if(msg.models[i].type=="packagen1"):
                #if(round((msg.models[i].pose.position.y),5)==-0.03203):
		if(((msg.models[i].pose.position.y)>-0.05) and ((msg.models[i].pose.position.y)<0.05)):
            ...
            elif (msg.models[i].type=="packagen2"):
                if((msg.models[i].pose.position.y<-0.1) and (msg.models[i].pose.position.y>-0.15)):
                     constop()
                     rospy.sleep(1) 
                     constart()  
            elif (msg.models[i].type=="packagen3"):
                if((msg.models[i].pose.position.y<-0.08) and (msg.models[i].pose.position.y>-0.09)):
                     constop()
                     rospy.sleep(1) 
                     constart()   
   	#print(msg.models[2].pose.position.y)
    	...
    	#we are getting the position of the logical camera centre but we have to get the position of the box when it reaches y == 0 so that we can lift it up and stop the converyer belt
...
def main():
   ...
   # constart()
    ...
    ur5_2_home_pose.position.x = -0.85
	...
	print("SUBSCRIBED TO LOGICAL CAMERA")
...
if __name__ == '__main__':
    sub = rospy.Subscriber("/eyrc/vb/logical_camera_2",LogicalCameraImage,callback1)
    main()
