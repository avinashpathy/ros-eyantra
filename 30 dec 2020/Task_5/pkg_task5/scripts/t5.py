...
def dispatched(flag):
	...
	time = get_time_str()
    	...
    	parameters = {"id":"OrdersDispatched", "Team Id":"#VB0865", "Unique Id": "aBCaRvEc", "Order ID":order_id,"City":ls[2], "Item":ls[4],"Priority":priority[ls[4]], "Dispatch Quantity":ls[1],"Cost":cost[ls[4]], "Dispatch Status":"YES", "Dispatch Date and Time":time} 
...
def estimated_time(pack_prio):
    timestamp = int(time.time())
    est = {"Medicine" : 1 , "Food" : 3, "Clothes" : 5 }
    value = datetime.datetime.fromtimestamp(timestamp)
    x = est[pack_prio]
    str_time = value.strftime('%Y-%m-(%d+%x)')
    return str_time
...
def shipped(condition):
	...
	est_time = estimated_time(ls[4])
    	...
    	parameters = {"id":"OrderShipped", "Team Id":"#VB0865", "Unique Id": "aBCaRvEc", "Order ID":order_ls[order_no],"City":ls[2], "Item":ls[4],"Priority":priority[ls[4]], "Shipped Quantity":ls[1],"Cost":cost[ls[4]], "Shipped Status":"YES", "Shipped Date and Time":time , "Estimated Time of Delivery":est_time } 
