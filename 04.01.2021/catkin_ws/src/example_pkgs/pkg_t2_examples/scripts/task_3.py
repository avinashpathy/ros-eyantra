...
class CartesianPath:
	...
	#this is called everytime a message is detected by the subscriber
        try:
            for i in range(3):
                if(msg.models[i].type=="packagen1" or msg.models[i].type=="packagen2" or msg.models[i].type=="packagen3"):
		
	            if(((msg.models[i].pose.position.y)>-0.05) and ((msg.models[i].pose.position.y)<0.05)):
             		#called the constop function twhen the package is in the range
		        self.constop(1)
		        count+=1							#counting the package
                        #multithreading *go_to_pose *turning on the belt
		        power_belt=threading.Thread(target=self.constart)			
                        pos_plan=threading.Thread(target=self.pos_des,args=count)
		        pos_plan.start()						#starting the pos_plan thread
 	       	        rospy.sleep(1)
		        power_belt.start()						#starting power_belt thread
	                pos_plan.join()							#waiting for pos_plan thread to complete
			self.dact_vac()							#deactivating the vaccum gripper
			flag=True							
		...
		#	self.constop(3)
    	    ...
    	    pass
    def pos_des(self,number):
	#function to giving destination coordinates for ur5
	...
	if(number==1):				#red package
        ...
        if(number==2):				#green package            
            
            self.ee_cartesian_translation(0.14,0,0)
            self.act_vac()

	    ur5_3_home_pose.position.x = 0.75
            ur5_3_home_pose.position.y = 0.0
            ur5_3_home_pose.position.z = 1.2
      
	
	
        if(number==3):				#blue package
               
            self.ee_cartesian_translation(-0.2,0,0)
            self.act_vac()

            ur5_3_home_pose.position.x = 0.04
            ur5_3_home_pose.position.y = -0.65
            ur5_3_home_pose.position.z = 1.2
           
        self.go_to_pose(ur5_3_home_pose)
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if(flag==True):			#ur5 going to home position after dropping package to bin
            ur5.go_to_pose(ur5_2_home_pose)
