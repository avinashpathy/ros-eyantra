...
class CartesianPath:
   	...
   	try:
		...
		if((msg.models[i].type=="packagen1") or (msg.models[i].type=="packagen2") or (msg.models[i].type=="packagen3")):
		    ...
		    if((msg.models[i].pose.position.y<0.05) and (msg.models[i].pose.position.y>-0.05)):
                        self.constop()
		        self.position(msg.models[i].pose.position.x,msg.models[i].pose.position.y)
        except:
	    pass
    def constart(self):
    ...
    def position(self,pos_x,pos_y):
        self.ee_cartesian_translation(pos_x+0.8,pos_y,0) 
        self.act_vac()
        self.constart()
        ur5_3_bin_pose = geometry_msgs.msg.Pose()
    
        
    
        #while(number_package!=0):
        if(number_package==3):
            ur5_3_bin_pose.position.x = 0.11
            ur5_3_bin_pose.position.y = 0.65
            ur5_3_bin_pose.position.z = 1.2            
        elif(number_package==2):
            ur5_3_bin_pose.position.x = 0.81
            ur5_3_bin_pose.position.y = 0.0
            ur5_3_bin_pose.position.z = 0.944
	elif(number_package==1):
            ur5_3_bin_pose.position.x = 0.04
            ur5_3_bin_pose.position.y = -0.65
            ur5_3_bin_pose.position.z = 1.2
        self.go_to_pose(ur5_3_bin_pose)
